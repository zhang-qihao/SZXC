...
<script>
  ...
  export default {
    ...
    mounted() {
        ...
        store.dispatch('GetTreeGrid',form).then((data) => {
            if(data&&data.options){
                var vueConstData = constData;
                let options = eval("("+data.options+")");
                console.log(options);
                let element = $("#" + this.id).kendoTreeList(options);
                this.widget = element.data('kendoTreeList');
            }else{
                this.$eframe.alertError("EfTreeGrid错误，未取到查询配置！");
            }
        }).catch((msg) => {
